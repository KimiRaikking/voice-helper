...
> ⚠️ **不论哪种方式,都必须放在纯英文路径**(如 `D:\voice-helper`)。**不要**放含中文/空格的目录(如 `D:\智能运维\...`)——会导致模型加载报「Not Found」。

### 方式 A:免安装绿色版(推荐给非研发,最省事)

不装 Python、不用 Git、不下模型、不碰代理。整个包(内置 Python + 依赖 + 模型)已由研发打好。

**安装流程:**

1. 从共享盘下载 **`voice-helper-portable.zip`**。
   > 下载地址:`<在此填共享盘/内部下载地址>`
2. **解压到纯英文路径**,如 `C:\voice-helper`(⚠️ 千万**不要**放在含中文/空格的目录,否则模型加载会失败)。
3. 进入解压出的文件夹,双击 **「启动语音输入.bat」**。
   - 会弹一下窗口(设置开机自启 + 启动),几秒后自动消失,属正常。
4. 看**屏幕右下角系统托盘**出现 **🎤** 图标(被折叠就点任务栏的 `^` 展开)。

**使用:** 在任意能打字的地方(微信/浏览器/文档/终端…),把光标放进输入框,**按住键盘右边的 Alt 键**说话,**松开**——文字自动出现。

**日常管理**(都在文件夹里双击):
- 已**自动开机自启**,平时不用管。
- 停用:双击 **「停止.bat」**;重新启动:双击「启动语音输入.bat」。
- 卸载:双击 **「卸载.bat」**,再删整个文件夹。

**装好后没出现 🎤 图标?**
双击 **「调试启动.bat」**(前台显示报错),把窗口里的报错截图/抄给研发。常见原因:
- 解压到了**中文路径** → 移到纯英文路径重试。
- 提示缺少运行库(`DLL load failed`)→ 装一下微软 VC++ 运行库(找 IT)。

> (研发)如何打包这个绿色版:见仓库 `PORTABLE.md`(`python build_portable.py`,几乎全程离线)。
...
> **绿色版用户**:直接双击包里的 `启动语音输入.bat` / `停止.bat` / `卸载.bat` 即可,无需以下命令。

以下命令用于源码安装方式,在 Git Bash、仓库目录里执行:
