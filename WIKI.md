...
1. 从共享盘下载 **`voice-helper-portable.zip`**(由研发打包好,内含 Python + 依赖 + 模型)。
   > 下载地址:`<在此填共享盘/内部下载地址>`
2. **解压到纯英文路径**,如 `C:\voice-helper`(不要放中文目录!)。
3. 双击 **「启动语音输入.bat」**。
4. 右下角托盘出现 🎤 图标后,在任意输入框**按住右 Alt 键**说话,松开即出字。

就这 3 步。已自动设为开机自启。停用双击「停止.bat」,卸载双击「卸载.bat」再删文件夹。

> 研发如何打包这个绿色版:见仓库 `PORTABLE.md`(`python build_portable.py`,几乎全程离线)。
